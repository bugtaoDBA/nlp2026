...
#3. ลำดับศักดิ์ของข้อมูล เพื่อจะหาค่าน้ำหนักข้อมูล เชื่อมต่อไปที่ 17
def get_physic_gate_preview(predicted_class, text):
    ...
    if "ร้ายแรง" in text or "จำนวนมาก" in text:
        base_weight = min(base_weight + 1.0, 10.0)        
    return base_weight 
# ส่วนการรัน
text = "การละเมิดสิทธิบัตรรายใหญ่"
weight = get_physic_gate_preview(1, text)
print(f"predicted Gate weight review: {weight/10.0}")
# run
text = "การละเมิดสิทธิบัตรรายใหญ่"
weight = get_physic_gate_preview(1,"text")
print(f"predicted Gate weight review: {weight/10.0}") 
